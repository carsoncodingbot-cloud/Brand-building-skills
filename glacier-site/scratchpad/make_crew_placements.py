"""Crew Trust ad in Meta's three placement ratios. Real tech photo (b2-09),
hands-over-logo positioning. 1:1 / 9:16 (safe zones) / 1.91:1. Deterministic."""
...
PHOTO="brand/jobs/enhanced/b2-09-tech-pump-repair.jpg"
...
def hero(img,W,top_h,centering=(0.5,0.35),start=0.42):
...
CHECKS=["Licensed, insured, background-checked",
        "The price in writing before work starts",
        "They clean up like they were never there"]
...
def sq():
    ...
    badge(img,d,int(70*SC),int(96*SC),"OUR CREW · ON A REAL JOB")
    d=ImageDraw.Draw(img,"RGBA")
    center(d,"YOU'RE NOT HIRING A LOGO.",f("Black",44),int(TOP-160*SC),(255,255,255,255),W//2)
    center(d,"YOU'RE HIRING WHOEVER",f("Black",44),int(TOP-108*SC),(120,205,246,255),W//2)
    center(d,"KNEELS NEXT TO YOUR SYSTEM.",f("Black",44),int(TOP-56*SC),(120,205,246,255),W//2)
    ...
    center(d,"FREE EXACT-PRICE ESTIMATES  ·  24/7 EMERGENCY",f("ExtraBold",22),ob_y+OB//2,(255,255,255,255),W//2)
    d=footer(img,d,W,H,H-FOOT,118,phfs=34,sitefs=20)
    img.convert("RGB").save("brand/placements/glc-crew-1x1.png",optimize=True)
    print("1:1 saved",img.size,"checks end",y//SC,"banner",ob_y//SC)
...
def story():
    ...
    TOP=int(1150*SC)
    hero(img,W,TOP,centering=(0.5,0.3),start=0.5)
    d=ImageDraw.Draw(img,"RGBA")
    d.rectangle([0,0,W,4*SC],fill=RED)
    eyebrow_brand(img,d,W,200,(200,226,246,255),fs=19,mh1=46)
    badge(img,d,int(70*SC),int(250*SC),"OUR CREW · ON A REAL JOB")
    d=ImageDraw.Draw(img,"RGBA")
    center(d,"YOU'RE NOT HIRING",f("Black",52),int(TOP-206*SC),(255,255,255,255),W//2)
    center(d,"A LOGO. YOU'RE HIRING",f("Black",52),int(TOP-142*SC),(120,205,246,255),W//2)
    center(d,"WHOEVER KNEELS",f("Black",52),int(TOP-78*SC),(120,205,246,255),W//2)
    # white content band
    d.rectangle([0,TOP,W,int(1436*SC)],fill=(255,255,255,255))
    center(d,"NEXT TO YOUR SYSTEM.",f("Black",40),int(TOP+44*SC),HEAD,W//2)
    y=TOP+int(86*SC)
    for t in CHECKS:
    ...
    img.convert("RGB").save("brand/placements/glc-crew-9x16.png",optimize=True)
    print("9:16 saved",img.size,"checks end",y//SC)
...
def wide():
    ...
    # left: photo down to bar, blended right edge
    PW=int(560*SC)
    ph=Image.open(PHOTO).convert("RGB")
    ph=ImageOps.fit(ph,(PW,bar_y),Image.LANCZOS,centering=(0.5,0.3))
    img.paste(ph,(0,0))
    ...
    d=ImageDraw.Draw(img,"RGBA")
    d.rectangle([0,0,W,4*SC],fill=RED)
    badge(img,d,int(24*SC),int(24*SC),"OUR CREW · ON A REAL JOB",fs=13)
    d=ImageDraw.Draw(img,"RGBA")
    # right column
    RX=int(620*SC); RCX=RX+(W-RX-int(50*SC))//2
    mark=Image.open("public/logo-mark.png").convert("RGBA"); mark=mark.crop(mark.getbbox())
    ...
    d.text((RX,int(112*SC)),"YOU'RE NOT HIRING A LOGO.",font=f("Black",31),fill=(255,255,255,255))
    d.text((RX,int(154*SC)),"YOU'RE HIRING WHOEVER",font=f("Black",31),fill=(120,205,246,255))
    d.text((RX,int(196*SC)),"KNEELS NEXT TO YOUR SYSTEM.",font=f("Black",31),fill=(120,205,246,255))
    y=int(262*SC)
    for t in CHECKS:
        cs=int(28*SC)
        ...
        d.text((RX+cs+int(14*SC),y-int(2*SC)),t,font=f("ExtraBold",18),fill=(235,244,252,255))
        y+=int(48*SC)
    tr=f("Bold",16)
    tt="FREE EXACT-PRICE ESTIMATES  ·  24/7 EMERGENCY"
    wt,_,bbt=T(d,tt,tr)
    d.rounded_rectangle([RX,int(430*SC),RX+wt+int(40*SC),int(472*SC)],radius=int(21*SC),fill=RED)
    d.text((RX+int(20*SC)-bbt[0],int(439*SC)),tt,font=tr,fill=(255,255,255,255))
    d=footer(img,d,W,H,bar_y,96,phfs=30,sitefs=19)
    img.convert("RGB").save("brand/placements/glc-crew-191.png",optimize=True)
...
import numpy as _np
for p,zone in [("brand/placements/glc-crew-1x1.png",(0,40,2160,170)),
               ("brand/placements/glc-crew-9x16.png",(0,320,2160,500)),
               ("brand/placements/glc-crew-191.png",(1100,40,2400,190))]:
