"""Funnel-congruent trust ad: the exact-price promise, mirroring the Instant Form
journey. AC-only imagery (b5-24). Outputs 4:5 + 1:1 + 9:16 + 1.91:1."""
...
PHOTO="brand/jobs/enhanced/b5-24-trane-condenser-brick-meter.jpg"
...
def hero(img,W,top_h,centering=(0.5,0.55),start=0.42):
...
CHECKS=["Tell us what your AC is doing — takes about a minute",
        "We call back fast with straight answers and options",
        "Exact price in writing — and it doesn't move"]
BADGE="REAL GLACIER INSTALL · SAN ANTONIO"
BANNER="AC REPAIR & REPLACEMENT  ·  FREE EXACT-PRICE ESTIMATES"
...
def feed():
    ...
    d=ImageDraw.Draw(img,"RGBA")
    center(d,"YOUR AC PRICE,",f("Black",58),int(TOP-186*SC),(255,255,255,255),W//2)
    center(d,"IN WRITING,",f("Black",58),int(TOP-126*SC),(120,205,246,255),W//2)
    center(d,"BEFORE WE START.",f("Black",58),int(TOP-66*SC),(120,205,246,255),W//2)
    ...
    ob_y=H-FOOT-OB
    kick=(y+ob_y)//2
    center(d,"One minute to fill out. Zero surprises after.",f("ExtraBold",24),kick,ICE,W//2)
    ...
    img.convert("RGB").save("brand/ad-hook-v2c-exactprice.png",optimize=True)
...
def sq():
    ...
    d=ImageDraw.Draw(img,"RGBA")
    center(d,"YOUR AC PRICE,",f("Black",46),int(TOP-160*SC),(255,255,255,255),W//2)
    center(d,"IN WRITING,",f("Black",46),int(TOP-108*SC),(120,205,246,255),W//2)
    center(d,"BEFORE WE START.",f("Black",46),int(TOP-56*SC),(120,205,246,255),W//2)
    d.rectangle([0,TOP,W,H],fill=(255,255,255,255))
    ...
    img.convert("RGB").save("brand/placements/glc-exact-1x1.png",optimize=True)
...
def story():
    ...
    d=ImageDraw.Draw(img,"RGBA")
    center(d,"YOUR AC PRICE,",f("Black",58),int(TOP-206*SC),(255,255,255,255),W//2)
    center(d,"IN WRITING,",f("Black",58),int(TOP-138*SC),(120,205,246,255),W//2)
    center(d,"BEFORE WE START.",f("Black",58),int(TOP-70*SC),(120,205,246,255),W//2)
    d.rectangle([0,TOP,W,int(1436*SC)],fill=(255,255,255,255))
    y=TOP+int(40*SC)
    for t in CHECKS:
        check_line(d,int(100*SC),y,t,fs=21,cs1=34)
        y+=int(62*SC)
    center(d,"One minute to fill out. Zero surprises after.",f("ExtraBold",22),int(TOP+236*SC),ICE,W//2)
    ...
    img.convert("RGB").save("brand/placements/glc-exact-9x16.png",optimize=True)
...
def wide():
    ...
    d.text((RX,int(112*SC)),"YOUR AC PRICE,",font=f("Black",33),fill=(255,255,255,255))
    d.text((RX,int(154*SC)),"IN WRITING,",font=f("Black",33),fill=(120,205,246,255))
    d.text((RX,int(196*SC)),"BEFORE WE START.",font=f("Black",33),fill=(120,205,246,255))
    y=int(262*SC)
    for t in CHECKS:
        cs=int(28*SC)
    ...
    img.convert("RGB").save("brand/placements/glc-exact-191.png",optimize=True)
...
import numpy as _np
for p,zone in [("brand/ad-hook-v2c-exactprice.png",(0,40,2160,170)),
               ("brand/placements/glc-exact-1x1.png",(0,40,2160,170)),
               ("brand/placements/glc-exact-9x16.png",(0,320,2160,500)),
               ("brand/placements/glc-exact-191.png",(1100,40,2400,190))]:
