...
import {
  Snowflake, Flame, Wind, Split, Droplet, Wrench, Shield, Building, Home,
  Check, Phone, ChevronRight, Star, Clock,
} from "@/components/Icons";

/* ---- Tailored to exactly the services Glacier offers ---- */
const SERVICES = [
  { id: "cooling", label: "AC & Cooling", note: "Repair, install & tune-ups", icon: Snowflake },
  { id: "heating", label: "Heating & Furnace", note: "Repair, heat pumps & installs", icon: Flame },
  { id: "iaq", label: "Indoor Air Quality", note: "Duct cleaning, filtration & UV", icon: Wind },
  { id: "mini-split", label: "Ductless Mini-Split", note: "Zoned comfort, any room", icon: Split },
  { id: "water-heater", label: "Water Heater", note: "Tank & tankless", icon: Droplet },
  { id: "plumbing", label: "Plumbing", note: "Leaks, drains & fixtures", icon: Wrench },
  { id: "maintenance", label: "Maintenance / Tune-Up", note: "Keep it running right", icon: Shield },
  { id: "commercial", label: "Commercial HVAC", note: "Offices, retail & more", icon: Building },
  { id: "other", label: "Not sure / Something else", note: "We'll help you figure it out", icon: Home },
];

const SITUATIONS = [
  { id: "down", label: "It's not working at all", note: "No cool, no heat, or no hot water", emergency: true },
  { id: "poor", label: "Working, but not right", note: "Weak, noisy, leaking, or inconsistent" },
  { id: "routine", label: "Routine maintenance", note: "A tune-up or seasonal check-up" },
  { id: "quote", label: "Quote for a new system", note: "Replacement or brand-new install" },
];

const TIMING = [
  { id: "emergency", label: "Emergency — ASAP", note: "Same-day priority dispatch", icon: Clock },
  { id: "soon", label: "Within a few days", note: "Sometime this week", icon: Clock },
  { id: "flexible", label: "I'm flexible", note: "Whatever works best", icon: Clock },
  { id: "quotes", label: "Just getting quotes", note: "Comparing my options", icon: Clock },
];
    ...
    return (
      <div className="relative overflow-hidden rounded-[var(--radius-card)] border border-ice-100 bg-white p-8 text-center shadow-[0_30px_60px_-30px_rgba(0,43,88,0.35)] sm:p-12">
        ...
        <p className="relative mx-auto mt-3 max-w-md text-slate-600">
          Thanks — a Glacier specialist will reach out shortly to confirm your time
          {svc ? <> for <span className="font-semibold text-navy-800">{svc.label.toLowerCase()}</span></> : null}.
          {isEmergency && <> Since this is urgent, we&apos;ll prioritize your call.</>}
        </p>
      ...
      <div className="bg-gradient-to-r from-navy-800 to-[#001a36] px-6 py-5 sm:px-8">
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-2 font-[family-name:var(--font-montserrat)] text-sm font-extrabold uppercase tracking-wide text-white">
            <Snowflake className="h-4 w-4 text-turquoise" /> Book Your Service
          </span>
      ...
      <div className="p-6 sm:p-8">
        ...
        {step === 1 && (
          <div key="s1" className="animate-step">
            ...
            <p className="mt-1 text-sm text-slate-500">
              ...
              This helps us send the right tech and parts.
            </p>
        ...
        {step === 3 && (
          <form key="s3" onSubmit={submit} className="animate-step">
            <h3 className="font-[family-name:var(--font-montserrat)] text-xl font-extrabold text-navy-800 sm:text-2xl">Where should we send your tech?</h3>
            <p className="mt-1 text-sm text-slate-500">
              {svc && <>For <span className="font-semibold text-navy-700">{svc.label.toLowerCase()}</span>. </>}
              We&apos;ll confirm your appointment by phone — usually within the hour.
            </p>
            ...
            <button type="submit" className="btn btn-primary mt-6 w-full text-base">
              {isEmergency ? "Request Priority Service" : "Book My Appointment"}
            </button>
