# Glacier Heating & Air — SEO & Growth Playbook

This is the honest, no-fluff plan for ranking in San Antonio, showing up in
...
- **Structured data (JSON-LD)** on every page:
  - `HVACBusiness` / `LocalBusiness` with NAP, geo coordinates, hours,
...
- **Local landing pages** — a dedicated page per city (Stone Oak, Alamo
  Heights, Boerne, New Braunfels, Schertz…) targeting "HVAC + [city]"
...
> domain `glacierheatingair.com`. They go fully live the moment that domain is
...
The map pack (the top 3 businesses on the map) drives the majority of HVAC
calls. It's ranked almost entirely by GBP signals, not your website.
- Claim & fully verify the profile at business.google.com.
- Categories: primary **"HVAC contractor"**, plus "Air conditioning
...
- Use the exact same NAP as in `lib/site.ts` on: Yelp, Facebook, Bing
  ...
  Pages, and HVAC directories.
- Get listed in San Antonio-specific directories (Chamber of Commerce,
...
- **Local press & blogs** — offer a "how to survive a San Antonio summer /
...
- "AC size calculator for a 2,000 sq ft San Antonio home"
...
1. [ ] Point `glacierheatingair.com` at the site (SSL auto).
